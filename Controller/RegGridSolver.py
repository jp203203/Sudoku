...
class RegGridSolver(GridSolver):
    ...
    def _create_pencil_marks(self):
        ...
        self._pencil_marks = [[[True for i in range(9)] for i in range(9)] for i in range(9)]

        for i in range(9):
            for j in range(9):
                # remove all marks from cells which are already filled out
                if self._grid[i][j] != 0:
                    self._pencil_marks[i][j] = [False for mark in range(9)]
    ...
    def _insert_cell(self, y_pos, x_pos, value):
        ...
        self._pencil_marks[y_pos][x_pos] = [False for mark in range(9)]
