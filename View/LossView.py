...
class LossView(View):
    ...
    def _create(self):
        ...
        win_message_lbl = Label(self._root, text="You lost the game...", font=Font(size=15, weight='bold'))
        win_message_lbl.grid(row=0, column=0, columnspan=2)
        ...
        # create retry button
        retry_btn = Button(self._root, text="retry", command=lambda:
                           [self._root.destroy(), self._game_interaction.retry()])
        retry_btn.grid(row=1, column=1)